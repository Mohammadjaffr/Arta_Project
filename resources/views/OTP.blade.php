...
<body>
    ...
    <div class="col-10 col-lg-5 custom-position container my-5 p-3 rounded-5 custom-shadow" style="background-color: #E7E7E7;min-width: 469px; max-width: 500px; max-height: fit-content;right: 15%">
        ...
        <style>
            ...
            <form method="post" action="{{route('verifyOtpAndLogin')}}" class="my-1 mx-3 p-2">
                @csrf
                <div class="text-end my-3 me-3 "> <h2>ادخل رقم التحقق</h2></div>
                <div class="text-end my-3 me-3 fw-bold text-black-50">لا تقلق، أدخل الايميل و رقم التحقق  أدناه لاستعادة كلمة المرور الخاصة بك </div>
                <div class="form-group text-end my-2" >
                    <label class="form-label">الايميل</label>
                    @error('email')
                        <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                    <input class="form-control py-2 rounded-4 custom-input my-2" style="width: 100%;padding: 10px 10px;color: #555;max-font-size: 33px"  name="email" id="email" type="email" value="{{ old('email') }}" >
                    <label class="form-label me-3">رقم التحقق</label>
                    @error('otp')
                    <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                    <input class="form-control py-2 rounded-4 custom-input " style="width: 100%;padding: 10px 10px;color: #555;max-font-size: 33px"  name="otp" id="otp" type="number" value="{{ old('otp') }}" >
